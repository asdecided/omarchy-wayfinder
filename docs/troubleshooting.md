...
The pinned `router-v1.1.0` exposes authenticated local project profiles. If the panel reports them as unavailable, confirm `wayfinder-router --version` and `wayfinder-router capabilities --json`, then check whether a different executable appears first on `PATH`. Do not work around the capability gate by editing QML or project state.
...
pins Router `1.1.0`; `2026.8.1` remains the immutable final DateVer release.
The next stronger release-gated claim requires a recorded
`2026.8.1` → `1.1.0` → `2026.8.1` → `1.1.0` cycle between checksum-verified

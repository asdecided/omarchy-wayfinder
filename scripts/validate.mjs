...
const readme = await readFile(path.join(root, "GUIDE.md"), "utf8");
...
assert.ok(service.includes('"/v1/value?workspace="'));
...
assert.ok(model.includes("function projectValue(raw)"));
assert.ok(model.includes("function projectBoundaryStats(report)"));
assert.ok(model.includes("function workspaceReceipts(report, workspaceId, limit, restrict)"));
...
assert.ok(widget.includes("PROJECT VALUE"));
assert.ok(widget.includes("ALL SAVINGS"));
assert.ok(widget.includes("Model.projectBaselineLabel(root.projectValue)"));
